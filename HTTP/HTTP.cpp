...
Apixu::HTTP::Client::Client(string userAgent) : userAgent(std::move(userAgent)) {}
...
inline string paramsToQuery(CURL* curl, const map<string, string> *params) {
    ...
    for (auto iter = params->begin(); iter != params->end();) {
        ...
        if (!key || !value) {
            throw Apixu::HTTP::Exception("Cannot escape query params");
        }
...
const Apixu::HTTP::Response *Apixu::HTTP::Client::get(const string& url, const map<string, string> *params) {
...
int Apixu::HTTP::Response::getStatus() const {
    return status;
}

const string &Apixu::HTTP::Response::getBody() const {
    return body;
}

Apixu::HTTP::Response::Response(int status, string body) : status(status), body(std::move(body)) {}
